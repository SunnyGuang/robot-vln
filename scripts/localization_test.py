...
class ObjectDistanceListener:
    def __init__(self):
        ...
        self.x_odom = 0
        self.y_odom = 0
        self.csv_path = os.path.join("/home/sunny/catkin_ws/src/project/memory/memory_localization.csv")
    ...
    def save_to_csv(self, class_name, probability, distance, coordinates, x_combined, y_combined):
        ...
        if os.stat(self.csv_path).st_size == 0 or not os.path.exists(self.csv_path):
            ...
            # Load existing data if probability is higher than 80%
            #if df.loc[mask, 'Probability'].iloc[0] > 0.8:
